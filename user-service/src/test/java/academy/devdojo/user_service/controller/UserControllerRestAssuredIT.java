...
import academy.devdojo.user_service.config.IntegrationTestConfig;
...
class UserControllerRestAssuredIT extends IntegrationTestConfig {
    ...
    @Order(1)
    void findAll_ReturnsAllUsers_WhenArgumentIsNull() throws IOException {
    ...
    @Sql(value = "/sql/user/init_one_user2.sql", executionPhase = Sql.ExecutionPhase.BEFORE_TEST_METHOD)
    @Sql(value = "/sql/user/clean_users.sql", executionPhase = Sql.ExecutionPhase.AFTER_TEST_METHOD)
    @Order(9)
